...
from tensorflow.keras.layers import Embedding, SimpleRNN, Dense  # Skirtingi neuroninių tinklų sluoksniai
...
max_words = 100000  # Nustatome maksimalų žodžių kiekį, kuris bus naudojamas analizei
...
# Sukuriame modelį
model = Sequential([
    Embedding(input_dim=max_words, output_dim=128),  # Sukuriame Embedding sluoksnį (žodžių įkrovimas)
...
model.fit(X_train_pad, y_train, epochs=20, batch_size=32, validation_split=0.2)  # Treniravimas su validacijos rinkiniu
